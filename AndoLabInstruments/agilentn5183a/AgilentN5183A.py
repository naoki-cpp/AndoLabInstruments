...
class AgilentN5183A(Instrument):
    def __init__(self, adapter, name="AgilentN5183A", **kwargs):
        super().__init__(
            adapter,
            name,
            includeSCPI = False,
    ...
    def configure_pulse_modulation(self, enable_pulse:bool, pulse_source:PulseSource = PulseSource.INTERNAL, internal_source:InternalSource = InternalSource.FREE_RUN, pulse_period:float = 3E-6, pulse_width = 1.5E-6):
        if(enable_pulse):
            ...
            self.write(':PULM:SOURce ' + pulse_source.value)
            if(pulse_source == self.PulseSource.INTERNAL):
                self.write(':PULM:Source:INTernal ' + internal_source.value)
                self.write(':PULM:INTernal:PERiod ' + str(pulse_period))
                self.write(':PULM:INTernal:PWIDth ' + str(pulse_width))
                self.write(':ROUTe:TOUT PSYNc')
        else:
